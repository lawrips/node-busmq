...
Queue.prototype._touch = function(conn) {
  ...
  this.qKeys.forEach(function(key) {
    conn.expire(key, _this._ttl, function(err, resp) {
      if (err) {
        // todo: the way this was previously written would crash the server in the event of a lost connection.
        // commenting out for now. revisit how to handle this properly.
//        _this.emit('error', "error setting key " + key + " expiration to " + _this._ttl + ": " + err);
        console.log('error', "error setting key " + key + " expiration to " + _this._ttl + ": " + err);
      }
    });
  })
};
